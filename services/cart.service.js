    ...
    for(let topping of dishInfo.additionalTopping){
     ...
     if(matching_dish ){
        ...
        if(!DishInCart){
         ...
         cart.total = (dishInfo.quantity * matching_dish.dishPrice) + total_topping_price;
         cart.imgUrl = imgUrl;
         cart.userId = userId;
        ...
        await Cart.findOneAndUpdate({dishId: dishId},{
            price: price,
...
if(cart){
if(cart._id == cartId){
       const {timeOfMeal,daysInWeek,NumberOfRepetition,additionalTopping} = newCart;
